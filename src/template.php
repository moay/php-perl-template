...
class Template {
    ...
    function Parse()
    {
        ...
        for ($i = 0; $i < count($chunks); $i++) {
            if (preg_match($regex2, $chunks[$i], $tag)) {
                ...
                if ($escape == "1") {
                    $escape = "HTML";
                } else if (empty($escape) || $escape == "NONE") {
                    $escape = 0;
                } else {
                ...
                case 'TMPL_ELSE':
                    if (end($curType) == "IF" || end($curType) == "UNLESS") {
                        // BEGIN DEBUG
                        if ($this->options['debug']) {
    ...
    function AddParam($arg, $value=NULL)
    {
        ...
        // or one argument (an associative array) (see README.md for details).
